...
// ─────────────────────────────────────────────────────────────────────────────
// Project Understanding Engine — Specifications + Execution Phases
// ─────────────────────────────────────────────────────────────────────────────

// --- Project Specifications ---
// Stores the full internal execution spec generated by the understanding engine.
// One spec per conversation; versioned when rebuilt.
export const projectSpecificationsTable = pgTable("project_specifications", {
  id: text("id").primaryKey(),
  conversationId: text("conversation_id").references(() => aiConversationsTable.id, { onDelete: "cascade" }),
  projectId: text("project_id").references(() => projectsTable.id, { onDelete: "set null" }),
...
export type ProjectSpecification = typeof projectSpecificationsTable.$inferSelect;

// --- Execution Phases ---
// Tracks each phase of spec execution: status, tasks, review results, artifacts.
export const executionPhasesTable = pgTable("execution_phases", {
  id: text("id").primaryKey(),
  specificationId: text("specification_id").references(() => projectSpecificationsTable.id, { onDelete: "cascade" }).notNull(),
...
export type ExecutionPhase = typeof executionPhasesTable.$inferSelect;
